...
if [[ "${1}" == "worker" ]]; then
  ...
  celery --app=superset.tasks.celery_app:app worker -Ofair -l WARNING
elif [[ "${1}" == "beat" ]]; then
  echo "Starting Celery beat..."
  celery --app=superset.tasks.celery_app:app beat --pidfile /tmp/celerybeat.pid -l WARNING -s "${SUPERSET_HOME}"/celerybeat-schedule
elif [[ "${1}" == "app" ]]; then
